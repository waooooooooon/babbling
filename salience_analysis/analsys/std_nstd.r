...
#データフレームの結合(GAM)
gam_all <-rbindCOrder(gam1,gam2,gamlast1,gamlast2)
rewa_all <- rbindCOrder(nrewa2,nrewa1)
DA_all <- rbindCOrder(DAt2,DAt1,DAlast2,DAlast1)
...
#GAM
g <- ggplot(gam_all,aes(x=sec,y=fit,group=group,color = group)) +
...
#setting of graf
  ...
  scale_color_manual(name="",values=c("Auditory feedback with STDP"="palevioletred4","Auditory feedback without STDP"="black"))+
